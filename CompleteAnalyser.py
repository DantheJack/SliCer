...
def mainCompleteAnalyser(sourceCodeScrolledText = None, resultTextArea=None, criterionVariable = 'a', criterionLine = 20, eraser = False):
    ...
    file.write(sourceCodeScrolledText.get("1.0", END))
    file.close()
    os.chdir(os.getcwd())                                       
...
#  #  #  #
    if(eraser == False):
        for line in targetFileAllTextLines:
            ...
            if (utility == 1):
                print(" --> " + "[ " + str(x) + " ]\t" + line)
                resultTextArea.insert(END, line, 'in') 
            else :
                print("     " + "[ " + str(x) + " ]\t" + line)
                resultTextArea.insert(END, line, 'out') 
        print()
#  #  #  #
    if(eraser == True):
        for line in targetFileAllTextLines:
            ...
            if (utility == 1):
                print(" oo> " + "[ " + str(x) + " ]\t" + line)
                resultTextArea.insert(END, line) 
            else :
                print("     " + "[ " + str(x) + " ]\t" + line)
                resultTextArea.insert(END, "\n") 
        print()
#  #  #  #
